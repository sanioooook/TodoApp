...
public class ShareTodoListUseCase : IShareTodoListUseCase
{
    ...
    public async Task<ShareTodoListResult> HandleAsync(ShareTodoListCommand command, CancellationToken ct)
    {
        ...
        var share = await _repository.AddShareAsync(command.ListId, command.TargetUserId, ct);
        if (!share)
            return new ShareTodoListResult { Success = false, Message = "Error while sharing TodoList", CodeResult = ResultCode.ServerError };
